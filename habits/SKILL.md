...
| 主题关键词 | 推断标签 |
|---|---|
| 骑行、跑步、游泳、健身、瑜伽、拉伸、健走、跳绳、打球 | 运动 |
| 阅读、读书、看书 | 学习 |
| 冥想、早睡、早起 | 健康 |
| 背单词、刷题、练琴、画画、写作 | 学习 |
| 做饭、家务、整理 | 生活 |

如果主题不在上述列表中，请根据语义合理推断，或不添加标签。

**时长推断规则：**
- "X 分钟" → `--duration=X`
- "X 小时" → `--duration=X*60`
- "X 小时 Y 分钟" / "X个半小时" → 换算为分钟
- 没有提到时长则不传 `--duration`
...
### 查看打卡记录
```
habits list [--topic=主题] [--tag=标签] [--date=YYYY-MM-DD] [--from="YYYY-MM-DD HH:mm:ss"] [--to="YYYY-MM-DD HH:mm:ss"] [--search=关键字] [--limit=数量]
```
- `--date`：查看某天的所有打卡
- `--from` / `--to`：时间范围筛选
- `--search`：按主题和备注模糊搜索
